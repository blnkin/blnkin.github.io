...
// Function to generate an array of 40 random, unique boxes
function getRandomBoxes() {
  const randomBoxes = [];
  while (randomBoxes.length < 60) {
...
function winGame() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      resetGame();
      localStorage.setItem("bonusgameResult", "win");
...
if (game.selectedBoxes.length === game.highlightedBoxes.length) {
  ...
  sounds.lose.play();
  setTimeout(() => {
    localStorage.setItem("bonusgameResult", "lose");
