...
// Function to generate an array of 16 random, unique boxes
function getRandomBoxes() {
  const randomBoxes = [];
  while (randomBoxes.length < 16) {
...
function winGame() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      resetGame();
      localStorage.setItem("9x9gameResult", "win");
...
if (game.selectedBoxes.length === game.highlightedBoxes.length) {
  ...
  sounds.lose.play();
  setTimeout(() => {
    localStorage.setItem("9x9gameResult", "lose");
