...
// Function to generate an array of 4 random, unique boxes
function getRandomBoxes() {
  const randomBoxes = [];
  while (randomBoxes.length < 4) {
...
function winGame() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      resetGame();
      localStorage.setItem("3x3gameResult", "win");
...
if (game.selectedBoxes.length === game.highlightedBoxes.length) {
  ...
  sounds.lose.play();
  setTimeout(() => {
    localStorage.setItem("3x3gameResult", "lose");
