...
// Function to generate an array of 10 random, unique boxes
function getRandomBoxes() {
  const randomBoxes = [];
  while (randomBoxes.length < 10) {
...
function winGame() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      resetGame();
      localStorage.setItem("7x7gameResult", "win");
...
if (game.selectedBoxes.length === game.highlightedBoxes.length) {
  ...
  sounds.lose.play();
  setTimeout(() => {
    localStorage.setItem("7x7gameResult", "lose");
