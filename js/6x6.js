...
// Function to generate an array of 8 random, unique boxes
function getRandomBoxes() {
  const randomBoxes = [];
  while (randomBoxes.length < 8) {
...
function winGame() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      resetGame();
      localStorage.setItem("6x6gameResult", "win");
...
if (game.selectedBoxes.length === game.highlightedBoxes.length) {
  ...
  sounds.lose.play();
  setTimeout(() => {
    localStorage.setItem("6x6gameResult", "lose");
